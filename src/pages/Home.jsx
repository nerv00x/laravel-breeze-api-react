...
const PartidosActivos = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://lapachanga-back.v2.test/api/partidos/this-week")
  ...
  const isBotonApostarDisabled = (horaPartido, fechaPartido) => {
    const fechaPartidoDate = new Date(fechaPartido);
    const horaPartidoDate = new Date(`2024-02-07 ${horaPartido}`);
    ...
    if (Math.abs(fechaPartidoDate - fechaActual) / (1000 * 60 * 60 * 24) <= 1) {
      return false;
    }
        ...
        <Card key={partido.id} className="mb-3">
          <Card.Body>
            ...
            <Button
              ...
              disabled={isBotonApostarDisabled(partido.hora, partido.fecha)}
            >
              Apostar
            </Button>
          </Card.Body>
